...
| المجموعة | النتيجة |
|---|---|
| بناء الـ Schema (9 جداول + FTS5 + 3 Views + 6 Triggers + 14 Indexes) | ✅ نجح |
| اختبارات إيجابية (T01–T22) | ✅ **22 / 22** |
| اختبارات سلبية — قيود ترفض بيانات فاسدة (N01–N13) | ✅ **13 / 13** |
| `pragma_foreign_key_check` | ✅ 0 مخالفات |
| سكريبت التشغيل الشامل `test-d1.sh` | ✅ 22 / 22 |
...
| categories | 11 (بذر كامل) | | stock_notifications | 1 |
| products | 4 | | b2b_articles | 1 |
| orders | 1 | | leads | 0 |
| order_items | 3 | | products_fts | 4 (متزامن) |
| shipping_zones | 24 (كل المحافظات) | | | |
...
| **T21** CASCADE / SET NULL | حذف المنتج يحذف إشعاراته ويبطّط `product_id` في عناصر الطلب |
| **N01–N13** القيود | رفض: سعر سالب، خصم>100، كمية 0، SKU مكرر، حالات عربية غير صالحة، FK غائب، مفضلة مكررة، محافظة مكررة، أنواع غير صالحة |
...
## 🔧 تعديل (2026-09-01): إزالة تقييمات وآراء العملاء نهائياً

بناءً على طلب العميل أُزيلت **تقييمات وآراء العملاء نهائياً** من المتجر والقاعدة:

- حُذف جدول `reviews` من `cloudflare/d1-schema.sql` و `migrations/0001_init.sql` و `supabase/schema.sql` (بما فيها RLS المرتبطة).
- حُذف عمودا `rating` و `reviews_count` من جدول `products`.
- أُزيلت واجهة التقييمات من الواجهة: `TestimonialsSection`، تبويب/نموذج التقييم في `ProductDetailModal`، نجمات/تقييمات في `ProductCard` و `HeroBanner` و `FlashDeals`، فلتر "التقييم" وترتيب "الأعلى تقييماً" في `ProductGrid`/`AdvancedFilters`، وبيانات `sampleReviews`.
- انخفض عدد اختبارات D1 من 23 إلى 22 إيجابياً ومن 14 إلى 13 سلبياً، والـ Schema من 10 إلى 9 جداول.
