...
expect_ok "تنفيذ 22 اختبار PASS/FAIL" run_sql_file cloudflare/d1-tests.sql
...
# N05 كمية صفر
expect_reject "N05 quantity = 0" ex "INSERT INTO order_items (order_id,product_name,quantity,unit_price,total_price) VALUES ('OMR-TEST-0001','س',0,10,0);"
# N06 حالة عربية غير مسموحة
expect_reject "N06 status عربي غير صالح" ex "UPDATE orders SET status='استلمت' WHERE id='OMR-TEST-0001';"
# N07 حالة دفع غير صالحة
expect_reject "N07 payment_status غير صالح" ex "UPDATE orders SET payment_status='maybe' WHERE id='OMR-TEST-0001';"
# N08 إشعار نفاد بدون أي وسيلة تواصل
expect_reject "N08 إشعار بدون email/phone/user" ex "INSERT INTO stock_notifications (product_id) VALUES ('p-rc-001');"
# N09 عنصر طلب لطلب غير موجود (FK)
expect_reject "N09 FK: order_id غير موجود" ex "INSERT INTO order_items (order_id,product_name,quantity,unit_price,total_price) VALUES ('NOPE-1','س',1,10,10);"
# N10 مفضلة مكررة (UNIQUE user+product)
expect_reject "N10 مفضلة مكررة" ex "INSERT INTO wishlists (user_id,product_id) VALUES ('u-test-merchant-1','p-rc-001');"
# N11 محافظة شحن مكررة
expect_reject "N11 محافظة مكررة" ex "INSERT INTO shipping_zones (governorate,governorate_ar) VALUES ('Gharbia','الغربية');"
# N12 نوع مستخدم غير صالح
expect_reject "N12 user_type غير صالح" ex "INSERT INTO profiles (email,full_name,user_type) VALUES ('n15@t.om','x','vip');"
# N13 فئة جملة غير صالحة
expect_reject "N13 wholesale_tier غير صالح" ex "INSERT INTO profiles (email,full_name,wholesale_tier) VALUES ('n16@t.om','x','tier9');"
