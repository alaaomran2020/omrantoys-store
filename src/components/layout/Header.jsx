...
export default function Header() {
  ...
  const { customer, isRegistered, clearCustomer } = useAuth();
  ...
  useEffect(() => {
    function handleClickOutside(e) {
      ...
      if (!insideDesktop && !insideMobile) setShowSearchResults(false);
  ...
  // قفل تمرير الصفحة عند فتح قائمة الموبايل
  useEffect(() => {
  ...
  const handleClearCustomer = () => {
    clearCustomer();
    localStorage.removeItem('omran_signup_seen');
  ...
  return (
    <header className="sticky top-0 z-40 bg-white shadow-sm transition-all">
      {/* Top Banner */}
      <div className="bg-gradient-to-r from-toy-red via-toy-purple to-toy-blue text-white text-[11px] sm:text-sm py-1.5 sm:py-2 px-3 sm:px-4 font-medium">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2">
          ...
          <div className="hidden md:flex items-center gap-4 text-xs shrink-0">
            ...
            <span className="opacity-40">|</span>
            <a href="tel:01555570269" className="flex items-center gap-1 hover:text-yellow-200 transition-colors"><PhoneCall className="w-3.5 h-3.5" /><span>01555570269</span></a>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 sm:py-3.5">
          ...
          <div className="flex items-center gap-1 sm:gap-2 md:gap-3">
            ...
            {isRegistered ? (
              <div className="relative hidden sm:block" ref={userMenuRef}>
                <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer bg-slate-900 text-white">
                  <User className="w-4 h-4" />
                  <span className="hidden md:inline max-w-[110px] truncate">{customer.fullName}</span>
                </button>

                {showUserMenu && (
                  <div className="absolute top-full left-0 mt-2 w-60 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50">
                    <div className="p-4 bg-slate-50 border-b border-slate-100">
                      <div className="font-black text-sm text-slate-900 truncate">{customer.fullName}</div>
                      <div className="text-xs text-slate-500 truncate" dir="ltr">{customer.phone}</div>
                      {customer.facebook && <div className="text-xs text-slate-500 truncate">فيسبوك: {customer.facebook}</div>}
                    </div>
                    <div className="p-2 space-y-1">
                      <button onClick={() => { setIsSignupOpen(true); setShowUserMenu(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"><User className="w-4 h-4" /> تعديل بياناتي</button>
                      <button onClick={() => { setIsTrackingOpen(true); setShowUserMenu(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"><Package className="w-4 h-4" /> تتبع طلبي</button>
                      ...
                      <button onClick={handleClearCustomer} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer"><X className="w-4 h-4" /> مسح بياناتي</button>
                    </div>
                  </div>
                )}
              </div>
            ) : (
              <button onClick={() => setIsSignupOpen(true)} className="hidden sm:flex items-center gap-1.5 bg-slate-900 hover:bg-slate-800 text-white px-3 py-2 rounded-xl text-xs font-bold transition-colors cursor-pointer"><User className="w-4 h-4" /><span>سجّل بياناتك</span></button>
            )}

            <button onClick={() => setIsWishlistOpen(true)} className="relative p-2.5 text-slate-600 hover:text-toy-red hover:bg-red-50 rounded-xl transition-all cursor-pointer" aria-label="المفضلة"><Heart className="w-5 h-5" />{wishlist.length > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-toy-red text-white text-[11px] font-black rounded-full flex items-center justify-center shadow-sm">{wishlist.length}</span>}</button>

            <button onClick={() => setIsCartOpen(true)} className="relative flex items-center gap-2 sm:gap-2.5 bg-toy-red hover:bg-rose-600 text-white px-2.5 sm:px-3.5 py-2 rounded-xl font-bold text-sm shadow-md shadow-toy-red/20 transition-all cursor-pointer active:scale-95">
              ...
              <div className="hidden sm:flex flex-col text-right leading-tight"><span className="text-[10px] font-normal text-rose-100">السلة</span><span className="text-xs font-black">{formatPrice(cartSubtotal)}</span></div>
            </button>

            <button onClick={() => setIsAdminOpen(true)} className="hidden sm:block p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-colors cursor-pointer" title="لوحة الإدارة"><Settings className="w-5 h-5" /></button>
          </div>
        </div>
        ...
        <div className="mt-2.5 md:hidden relative">
          <div ref={mobileSearchRef} className="relative w-full">
            <input type="text" value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setShowSearchResults(true); }} onFocus={() => setShowSearchResults(true)} placeholder="ابحث عن ألعاب، بالونات، حفلات..." className="w-full pl-9 pr-10 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-toy-red/20" />
      ...
      <div className="border-t border-slate-100 bg-white/95 backdrop-blur-sm shadow-sm overflow-x-auto no-scrollbar">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 flex items-center gap-2 whitespace-nowrap">
          {categories.map((cat) => {
            const isActive = selectedCategory === cat.id;
            return <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shrink-0 ${isActive ? 'bg-slate-900 text-white shadow-sm' : 'bg-slate-100/70 text-slate-700 hover:bg-slate-200/80 hover:text-slate-900'}`}><span>{cat.name}</span>{cat.id !== 'all' && <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${isActive ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-600'}`}>{cat.count}</span>}</button>;
          })}
        </div>
      </div>
              ...
              <div className="mt-4 space-y-2.5">
                {isRegistered ? (
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <div className="font-bold text-sm text-slate-900">{customer.fullName}</div>
                ...
                {isRegistered && <button onClick={handleClearCustomer} className="w-full flex items-center gap-3 p-3.5 rounded-xl bg-rose-50 text-rose-700 font-bold text-sm"><X className="w-5 h-5" /> مسح بياناتي</button>}
              </div>

              <div className="mt-5"><h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5"><LayoutGrid className="w-3.5 h-3.5" /> تصفح الأقسام</h5><div className="space-y-1">{categories.map(cat => <button key={cat.id} onClick={() => { setSelectedCategory(cat.id); setIsMobileMenuOpen(false); }} className={`w-full text-right px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors flex items-center justify-between ${selectedCategory === cat.id ? 'bg-toy-red text-white' : 'hover:bg-slate-50 text-slate-700'}`}><span>{cat.name}</span><span className="text-xs opacity-75">{cat.count} لعبة</span></button>)}</div></div>
            </div>
            <div className="pt-5 border-t border-slate-100 text-center text-xs text-slate-400"><p>متجر عمران للألعاب © 2026 - مصر 🇪🇬</p><p className="mt-1">جميع الأسعار بالجنيه المصري</p></div>
          </div>
        </div>
      )}
    </header>
