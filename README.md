...
## 🔐 لوحة الإدارة المؤمَّنة (جديد)

دخول المدراء عبر **كود OTP من واتساب** (بدون كلمات مرور) على المسار `/#/admin`،
مبني بالكامل على **Cloudflare Workers + D1** (بلا أي اعتماد على Vercel):

- **مصادقة واتساب**: كود 6 أرقام + رابط سحري أحادي الاستخدام عبر WhatsApp Cloud API مع Webhook مُتحقَّق التوقيع — [الهندسة الكاملة](docs/whatsapp-auth-architecture.md)
- **صلاحيات محدودة (RBAC حقل-بحقل)**: الموظف يعدّل *الاسم/السعر/الوصف/الصور فقط* — أي حقل آخر (`stock_quantity`, `sku`, `is_active`…) يُرفض بـ 403 ويُسجَّل في `admin_audit_log`، والحذف للمالك حصراً
- **جلسات آمنة**: كوكي `HttpOnly + Secure + SameSite=Strict`، يُخزَّن في D1 كـ SHA-256 معلَّب فقط، مع حدود معدل (3 أكواد/15د للرقم) وحد 5 محاولات للتحقق
- **واجهة Dark Digital Brutalism**: خلفية Slate، أزرق كهربائي للعناصر النشطة، وأصفر Sunbeam حصرياً لزر الحفظ

حسابات تجريبية (محلياً مع `AUTH_DEV_MODE=1` يظهر الكود في الواجهة): المالك `+201000000001` — موظف محدود `+201000000002`.

```bash
npm run db:d1:migrate && npm run cf:dev   # ثم افتح http://localhost:8787/#/admin
```
